...
def main():
    ...
    service = AccessControlService('data/database/license_plates.db')
    print("   ✓ Service initialized with database: data/database/license_plates.db")
    ...
    print("\n   Database file: data/database/license_plates.db")
    print("   Location: data/database/ (relative to project root)")
    print("\n   Tables created:")
...
'''
    ...
    summary = {
        ...
        'Database': 'data/database/license_plates.db (Created)',
   ...
   2. Integrate with your main script:
      - See scripts/main_with_authorization.py for complete example
      - Use service.process_plate() in your detection loop

   3. Process your video with authorization visualization:
      python scripts/main_with_authorization.py
   ...
   5. Review access logs in database:
      - Database: data/database/license_plates.db
      - Table: access_log
""")
